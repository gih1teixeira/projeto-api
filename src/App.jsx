...
function App() {
  ...
  }, []); // Array vazia para executar apenas UMA vez ao abrir a tela.
  return (
    <>
      <div>
        <div>
          <h2>Tarefas vindas da API</h2>
          <p>Consumindo dados de JSONPLACEHOLDERS via fetch e useEffect</p>
          {carregando ? (
            <div>
              Carregando...
            </div>
            ...
            <ul>
              {tarefas.map((item) => (
                <li key={item.id}>{item.title}{item.completed ? 'Concluído' : 'Pendente'}</li>
              ))}
            </ul>
          )}
        </div>
